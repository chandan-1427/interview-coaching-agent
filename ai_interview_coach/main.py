...
class InterviewState(TypedDict):
    ...
    question: str
    user_memory: str        # INJECTED: Past user context from Mem0
    exa_research: str       # INJECTED: Real-world engineering context from Exa
    intent_summary: str
...
_skills_cache: dict[str, str] = {}  # SCALING FIX: In-memory prompt cache
...
async def analyzer_node(state: InterviewState) -> dict[str, str]:
    ...
    # 1. FOUNDER METHODOLOGY: Exa Search for real-world grounding
    exa_context = "No external research needed."
    if exa_client:
        try:
            # Search for real engineering blog posts to make the story authentic
    ...
    if global_llm is None: raise RuntimeError("LLM not initialized")
    
    # FOUNDER METHODOLOGY: Inject Mem0 history into context
    context = (
        f"User History: {state.get('user_memory', 'None')}\n"
        f"Question: {state.get('question', '')}"
    )
    
    response = await global_llm.ainvoke([
...
async def polish_node(state: InterviewState) -> dict[str, str]:
    """Final synthesis for verbal delivery."""
    print("\n" + "="*50)
...
async def initialize_all(env: dict[str, str] | None = None) -> None:
    """Initialize LLM, Graph, Exa, and Mem0 (Founder Pattern)."""
    ...
    # 3. Initialize LLM (Production specs)
...
async def run_agent(messages: list[dict[str, str]]) -> Any:
    ...
    user_id = "bindu_candidate_default"  # Replace with actual user ID system if needed
    ...
    # FOUNDER METHODOLOGY: Fetch Memory
    past_memory = ""
    if mem0_client:
        try:
    ...
    try:
        ...
        return f"Operational Error: Unable to process the interview request at this time."
