...
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting ai-interview-coach in development mode..."
	@uv run watchfiles "python -m ai_interview_coach" ai_interview_coach/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t ai_interview_coach:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t ai_interview_coach:latest .
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi ai_interview_coach:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag ai_interview_coach:latest chandan1427s/ai_interview_coach:latest
	@docker push chandan1427s/ai_interview_coach:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep ai_interview_coach || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep ai_interview_coach || echo "No images found"
	@echo "\nAgent Health:"
